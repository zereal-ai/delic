...
;; Test signatures for validation - defined locally in tests to avoid unused warnings

(deftest fn-module-creation-test
  (testing "fn-module creation and basic properties"
    ;; Create test signature manually to avoid compilation issues
    (let [sig-map {:inputs [:input] :outputs [:output]}
          schema (#'sig/build-schema sig-map)
          SimpleProcessing (with-meta sig-map {:malli/schema schema})]
...
(deftest wrap-fn-convenience-test
  (testing "wrap-fn convenience function"
    ...
    (let [sig-map {:inputs [:input] :outputs [:output]}
          schema (#'sig/build-schema sig-map)
          SimpleProcessing (with-meta sig-map {:malli/schema schema})
          simple-fn (fn [_inputs] {:result "done"})]
...
(deftest signature-validation-test
  (testing "input validation with signatures"
    ...
    (let [sig-map {:inputs [:input] :outputs [:output]}
          schema (#'sig/build-schema sig-map)
          SimpleProcessing (with-meta sig-map {:malli/schema schema})
          validated-module (mod/fn-module
